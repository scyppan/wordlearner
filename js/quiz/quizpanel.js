...
function refreshquizpanel() {
  ...
  if (!Array.isArray(quizplan) || quizplan.length === 0) {
    ...
    empty.textContent = 'No quiz selections yet. Choose ranges or items from the Lessons tab.'
...
function producequizfromplan() {
  ...
  if (collected.length === 0) {
    showquizstatusmodal('No items were found for these planner selections.\nCheck lesson progress and try again.')
    return
  }
...
function ensurequizzesarray() {
  ...
  var stored = typeof retrievedata === 'function'
    ? retrievedata('quizzes')
    : null
...
function collectrangefromentry(entry, collected, seen) {
  ...
  for (var i = 0; i < lesson.items.length; i++) {
    ...
    var bucket = typeof getitemconfidencebucket === 'function'
...
function findlessonforentry(entry) {
  ...
  for (var i = 0; i < fullset.length; i++) {
    ...
    var lnum = typeof lesson.lessonnumber === 'number' ? lesson.lessonnumber : null
...
function itemmatchesrange(bucket, range) {
  ...
  if (range === 'highplus') {
    return bucket === 'untested' || bucket === 'low' || bucket === 'mid' || bucket === 'high'
  }
