...
function buildlessonslist() {
  ...
  lessons.sort(function (a, b) {
    var astr = a && a.lessonnumber != null ? String(a.lessonnumber).trim() : "";
    var bstr = b && b.lessonnumber != null ? String(b.lessonnumber).trim() : "";

    if (!astr && !bstr) return 0;
    if (!astr) return 1;
    if (!bstr) return -1;

    var an = parseInt(astr, 10);
    var bn = parseInt(bstr, 10);
...
function renderlessondetailpanel(lesson, lessonindex) {
  ...
  btnuntested.textContent = "No confidence items: " + stats.untested;
  ...
  btnlow.textContent = "Low confidence items: " + stats.low;
  ...
  btnmid.textContent = "Middle confidence items: " + stats.mid;
  ...
  btnhigh.textContent = "High confidence items: " + stats.high;
  ...
  btnsolid.textContent = "Solidified items: " + stats.solid;
...
function createlessonli(lesson, index) {
  ...
  var num = "";
  if (lesson && lesson.lessonnumber != null) {
    num = String(lesson.lessonnumber).trim();
  }

  var name =
    lesson && typeof lesson.lessonname === "string"
      ? lesson.lessonname
      : "Lesson";
...
function createlessondetailitem(lesson, item, index) {
  ...
  var roman =
    item && typeof item.romanization === "string" ? item.romanization : "";
...
function getlessonlabel(lesson) {
  ...
  var name =
    typeof lesson.lessonname === "string" ? lesson.lessonname : "Lesson";
